...
// ************************************************************
// Find knn correspondence with normals
// ************************************************************
void findCorrespondences (Eigen::MatrixXd& in_pFixed, Eigen::MatrixXd& in_qMoving,Eigen::MatrixXd& pNormals, Eigen::MatrixXd& qNormals,Eigen::MatrixXd& out_pFixed, Eigen::MatrixXd& out_qMoving, double BAD_P_FILTER, double ANGLE_THRESH){
	...
	for (int i = 0; i < nQueryPts; i++)
	{
		...
		// std::cout << "* angle normals  = " << std::acos(normalsDotProd)* 180.0 / M_PI << '\n';
		// std::cout << "* dot prod  = " << normalsDotProd << " > angle thresh" << angleThresh << '\n';
		if (correspArray(i,1) < filterMaxDist)
		{
			Eigen::Vector3d thisPNormal = pNormals.col(thisIdx);
			Eigen::Vector3d thisQNormal = qNormals.col(i);

			// compute the dot product of the 2 normals
			double normalsDotProd = thisPNormal.dot(thisQNormal);

			// double degrees = 5.0;
			double angleThresh = std::cos ( ANGLE_THRESH * M_PI / 180.0 );

			if (normalsDotProd > angleThresh)
			{
...
void getIcpIteration(Eigen::MatrixXd& in_pFixed, Eigen::MatrixXd& in_qMoving, Eigen::MatrixXd& pNormals, Eigen::MatrixXd& qNormals, Eigen::Matrix3d& rotationMatrix, Eigen::Vector3d& translationVec, double& err, double BAD_P_FILTER, double ANGLE_THRESH)
{
	int numPts = in_qMoving.cols();

	Eigen::MatrixXd out_p(Eigen::MatrixXd::Zero(3,numPts));
	Eigen::MatrixXd out_q(Eigen::MatrixXd::Zero(3,numPts));
	findCorrespondences (in_pFixed, in_qMoving, pNormals, qNormals, out_p, out_q, BAD_P_FILTER, ANGLE_THRESH);
...
void applyICP(Eigen::MatrixXd& in_pFixed, Eigen::MatrixXd& in_qMoving, Eigen::MatrixXd& pNormals, Eigen::MatrixXd& qNormals, Eigen::MatrixXd& final_qMoving, int& out_numIter, double& out_finErr, int maxIter, double errThresh, double BAD_P_FILTER, double ANGLE_THRESH)
{
	std::ofstream outputFile;
	outputFile.open ("icpErrors.txt");
	int numPts = in_qMoving.cols();
	...
	for( i = 0; i <= maxIter; i++ )
	{
		...
		getIcpIteration(in_pFixed, updated_qMoving, pNormals, qNormals, rotMat, translVec, thisErr, BAD_P_FILTER, ANGLE_THRESH);
